...
function checkBeanCount(event: Event | null = null) {
  ...
  APIConnector.getBohnenCount(formData.get("bohne") as string).then((data) => {
    if (data - beansToBeUsed < 0) {
      throwError(
        "beans-low",
        "Bitte fülle die Bohnen nach. Es sind nur " +
          data +
          "g Bohnen vorhanden.",
      );
      setTimeout(() => {
    ...
    if (data - beansToBeUsed >= 0 && data - beansToBeUsed <= 20) {
      warn(
        ...
        " Es werden nur noch " +
          (data - beansToBeUsed) +
          "g Bohnen vorhanden sein. Aktuell noch " +
          data +
          "g Bohnen.",
      );
    } else {
...
function getRecipe() {
  ...
  APIConnector.getRezept(
    formData.get("method") as string,
    formData.get("bohne") as string,
  ).then((data) => {
    // if empty object: no recipe found reset input fields
    if (Object.keys(data).length == 0) {
      (
        form.value?.querySelector(
          'input[name="Bohnenmenge"]',
        ) as HTMLInputElement
      ).value = "";
      (
        form.value?.querySelector('input[name="mahlgrad"]') as HTMLInputElement
      ).value = "";
      (
        form.value?.querySelector(
          'input[name="Getränkemenge"]',
        ) as HTMLInputElement
      ).value = "";
      (
        form.value?.querySelector(
          'input[name="brühtemperatur"]',
        ) as HTMLInputElement
      ).value = "";
      return;
    }
    (
      form.value?.querySelector('input[name="Bohnenmenge"]') as HTMLInputElement
    ).value = data.Bohnenmenge.toString();
    (
      form.value?.querySelector('input[name="mahlgrad"]') as HTMLInputElement
    ).value = data.Mahlgrad.toString();
    (
      form.value?.querySelector(
        'input[name="Getränkemenge"]',
      ) as HTMLInputElement
    ).value = data.Getränkemenge.toString();
    (
      form.value?.querySelector(
        'input[name="brühtemperatur"]',
      ) as HTMLInputElement
    ).value = data.Brühtemperatur.toString();
...
async function saveAsRecipe() {
  ...
  success(
    "recipe-created",
    "Rezept für " +
      formBrühung.BohnenName +
      " und " +
      formBrühung.BrühmethodenName +
      " gespeichert",
  );
  setTimeout(() => {
...
<template>
  <form id="brüh-page" ref="form" @submit="appendBrühung($event)">
    <div id="selection-area" class="section-wrapper">
      ...
      <div class="flex-div">
        <SelectComponent
          ...
          :fetch-function="
            () => {
              return APIConnector.getMethods();
            }
          "
        />
        <SelectComponent
          ...
          :fetch-function="
            () => {
              return APIConnector.getBohnenNames();
            }
          "
        />
      </div>
        ...
        <label for="Bohnenmenge">
          ...
          <div>
            <input
              ...
              required
              name="Bohnenmenge"
              placeholder="Bohnenmenge"
              @input="checkBeanCount"
              @change="checkBeanCount"
              inputmode="numeric"
            />
            <span> g </span>
          </div>
    ...
    <div id="info-center">
      <InfoComponent
        v-for="(item, index) in info.success"
        :key="index"
        :message="item"
        status="success"
      />
      <InfoComponent
        @close="release('warnings', index as string)"
        v-for="(item, index) in info.warnings"
        :key="index"
        :message="item"
        status="warning"
      />
      <InfoComponent
        v-for="(item, index) in info.errors"
        :key="index"
        :message="item"
        status="error"
      />
